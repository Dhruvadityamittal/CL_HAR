...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size', default=200, type=int, dest='sz_batch', help='Number of samples per batch.')  # 150
    ...
    parser.add_argument('--weight-decay', default=1e-3, type=float, help='Weight decay setting')
    ...
    pth_rst_exp = '/home/dmittal/Desktop/CGCD-main/src/Saved_Models/Initial/Edison_DeepLSTM/' + args.dataset + '/' #+ args.model + '_sp_' + str(args.use_split_modlue) + '_gm_' + str(args.use_GM_clustering) + '_' + args.exp
    ...
    if args.dataset =='wisdm':
        ...
        pth_dataset = '/home/dmittal/Desktop/CGCD-main/src/HAR_data/oppo/'
    ...
    for epoch in range(0,args.nb_epochs):  #args.nb_epochs
        ...
        for batch_idx, (x, y, z) in pbar:
            ...
            ce_loss = loss_1(log_p, y_query).to(device)
    ...
    acc_o_base, f1_o_base, acc_n, f1_n, _, _ = get_accuracy_old_new_LSTM(model, dlod_test_0, nb_classes, nb_classes, trained_prototypes_dict)
    ...
    acc_o, f1_o, acc_n, f1_n, _, _ = get_accuracy_old_new_LSTM(model, dlod_test, nb_classes, nb_classes_now, trained_prototypes_dict)
  

    wandb.log({"M1-TA2_Old": acc_o})   # Logging Initial Step Accuracy.
    wandb.log({"M1-TF2_Old": f1_o})  # Logging Initial Step F1 Score.
    ...
    pth_rst_exp = '/home/dmittal/Desktop/CGCD-main/src/Saved_Models/Incremental/Edison_DeepLSTM/' + args.dataset + '/' #+ args.model + '_sp_' + str(args.use_split_modlue) + '_gm_' + str(args.use_GM_clustering) + '_' + args.exp
    os.makedirs(pth_rst_exp, exist_ok=True)
    ...
    for epoch in range(0,args.nb_epochs):  #args.nb_epochs
        ...
        for x, y, z in dlod_tr_only_new:
            ...
            log_p = sftmax(-dists)
            ce_loss = loss_1(log_p, query_set_labels)
            ...
            model_now.train()
        print("Epoch {} CE Loss {:.5f} Constastive Loss {:.5f} Accuracy ".format(epoch, total_ce_loss/nsteps, total_contrastive_loss/nsteps ))
    ...
    wandb.log({"M2-TFA":  acc_o_base - acc_o})  
    wandb.log({"M2-TFF1": f1_o_base - f1_n})  
